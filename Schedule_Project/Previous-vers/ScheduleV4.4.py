...
img =Image.open('C:\\Users\\ahmed\\Desktop\\Schedule_Project\\welcome.jpg')
bg = ImageTk.PhotoImage(img)
main_label = tk.Label(window,image=bg)
...
money_frame = tk.Frame(main_label, bg = bgcolor)


money_month_frame = tk.Frame(main_label,bg = bgcolor)
...
calendar_frame = tk.Frame(main_label,
                          background = bgcolor)
...
def calculate_hours():
    ...
    def update_companies(event):
        ...
        query = "SELECT distinct school FROM teaching_schedule WHERE MONTH(date) = %s;"
    ...
    def update_schools(event):
        ...
        query = "SELECT distinct SUBSTRING(class,1,3) FROM teaching_schedule WHERE MONTH(date) = %s AND school = %s;"
        cursor.execute(query,(month_number,company_var.get()))
    ...
    def calculate_hours_worked(selected_month, selected_company,selected_school):
        ...
        if selected_company == "ALL":
            ...
            if selected_school == "ALL":
                query = "SELECT * FROM teaching_schedule WHERE MONTH(date) = %s AND school = %s "
                cursor.execute(query, (month_number,selected_company))
            else:
                query = "SELECT * FROM teaching_schedule WHERE MONTH(date) = %s AND school = %s AND class like %s"
                cursor.execute(query, (month_number,selected_company,f"%{selected_school}%"))
...
def calculate_totals():
    def calculate(time):
        ...
        total_hours = round(total_hours, 2)
        return total_hours, total_money, received_money, expected_money, time

    total_hours, total_money, received_money, expected_money, time = calculate("week")
    (
        total_hours_month,
        total_money_month,
...
def open_info_window(event):
    ...
    def fill_window(window, window_name):
        if class_info:
            ...
            paid_label = tk.Label(window, text="Rate:")
...
def update_loading_bar():
    ...
    if progress < 100:
        progress += 1
        loading_canvas.coords(loading_bar, 0, 0, progress * 4, 20)
        loading_canvas.itemconfig(loading_text, text=f"{progress}%")
        loading_canvas.update()
        loading_canvas.after(20, update_loading_bar)
        print(progress)
    else:
...
# Create a Canvas widget for the loading bar
loading_canvas = tk.Canvas(main_label, width=400, height=40,background=bgcolor)
loading_canvas.place(x = 500,y = 700)

# Create the loading bar and text
loading_bar = loading_canvas.create_rectangle(0, 0, 0, 20, fill="white")
loading_text = loading_canvas.create_text(200, 35, text="0%", font=("Helvetica", 12,),fill="white")
...
refresh_label = ttk.Label(main_label,image=refresh_btn)
# refresh_label.grid(row=2, column=2, columnspan=3, padx=10, pady=10)

refres_button= tk.Button(main_label, image=refresh_btn,command= filter_classes,
