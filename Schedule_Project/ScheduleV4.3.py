...
def update_paid_status():
    ...
    confirmation = messagebox.askyesno(
        "Confirm Update",
        f"Are you sure you want to update 'paid' status for all classes between {formatted_start_date} and {formatted_end_date} to 'y'?",
    )
    
    if confirmation:
        # Update the 'paid' status for the selected classes
        update_query = "UPDATE teaching_schedule SET paid = 'yes' WHERE date BETWEEN %s AND %s"
